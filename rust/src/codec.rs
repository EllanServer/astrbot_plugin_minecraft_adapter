...
impl ObservationRecordCodec {
    ...
    pub fn normalize_record(&self, py: Python, record: &Bound<PyAny>) -> PyResult<()> {
        ...
        let compacted_context = compact_py_dict(
            py,
            &context_dict,
            self.max_raw_fields,
        ...
        if self.include_raw {
            ...
            let compacted_raw =
                compact_py_dict(py, &raw_dict, self.max_raw_fields, self.max_content_length)?;
    ...
    pub fn dedupe_key(&self, record: &Bound<PyAny>) -> PyResult<String> {
        ...
        let identity: String = record.getattr("identity")?.extract().unwrap_or_default();
        let content: String = record.getattr("content")?.extract()?;
        let timestamp: i64 = record.getattr("timestamp")?.extract()?;
...
fn py_any_to_json(obj: &Bound<PyAny>) -> PyResult<Value> {
    ...
    let s: String = obj.str()?.extract().unwrap_or_else(|_| obj.to_string());
    Ok(Value::String(s))
}
