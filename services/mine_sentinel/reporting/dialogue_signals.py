...
class DialogueSignalCollector:
    ...
    def add(
        ...
        fingerprint = signal_fingerprint(record, normalized_text)
        group.signal_fingerprints.add(fingerprint)
        group.message_fingerprints.add(message_fingerprint(record.content))
        if player:
        ...
        if matched_terms(normalized_text, rule.urgent_terms):
            group.urgent += 1
...
def signal_fingerprint(record: ObservationRecord, normalized_text: str) -> str:
    player = record_player(record).lower()
    return f"{player}|{message_fingerprint(normalized_text)}"
