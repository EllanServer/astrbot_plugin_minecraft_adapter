...
class AIReportPromptBuilder:
    ...
    def prompt_text(
        ...
        return (
            ...
            "样本消息、review_evidence 审查证据），你必须在 chat_summary 字段输出面向管理员的"
            "聊天热点归纳：先说活跃玩家和讨论主题（1-2 句），再贴出需要审查的聊天证据"
            "（review_evidence 里的 player/message/spam_type/time_text，按时间序列出原文，"
            "不要省略消息内容，让管理员能直接看到违规原文）；没有聊天记录时输出空字符串。"
