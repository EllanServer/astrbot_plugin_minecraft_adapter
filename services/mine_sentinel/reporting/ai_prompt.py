...
class AIReportPromptBuilder:
    ...
    def prompt_text(
        ...
        return (
            ...
            "样本消息、flood_players 刷屏玩家、review_evidence 审查证据）。刷屏定义：同一ID"
            "短时间集中发送大量重复或高度相似的信息。你必须在 chat_summary 字段输出面向管理员的"
            "聊天热点归纳：先说活跃玩家和讨论主题（1-2 句）；如果 flood_players 非空，必须"
            "贴出刷屏玩家、刷屏类型（high_frequency 高频/repeat_content 重复/meaningless 无意义）、"
            "时间窗口、消息数和样本原文（不要省略消息内容）；再贴出 review_evidence 里其他"
            "需要审查的聊天证据（关键词命中，按时间序列出 player/message/hit_keys/time_text）；"
            "没有聊天记录时输出空字符串。"
