...
def build_metric_context(records: list[ObservationRecord]) -> dict[str, dict[str, Any]]:
    ...
    accumulator = MetricContextAccumulator()
    for record in records:
        accumulator.add(record)
    return accumulator.build()


class MetricContextAccumulator:
    """Incrementally aggregate metrics while another pipeline pass is running."""
    ...
    def add(self, record: ObservationRecord):
        ...
        item = self._aggregates.setdefault(location, _new_metric_context(location))
        item["samples"] += 1
    ...
    def build(self) -> dict[str, dict[str, Any]]:
        return {
            ...
            for location, item in sorted(self._aggregates.items())
        }
