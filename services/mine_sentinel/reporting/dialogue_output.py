...
class DialogueIssueBuilder:
    ...
    def signals(self, groups: list[DialogueSignalGroup]) -> list[dict[str, Any]]:
        max_findings = max(0, int(self.config.dialogue.max_findings))
        if max_findings <= 0:
            return []
        heap = []
        for index, group in enumerate(groups):
            ...
            if not signal:
                continue
            entry = (*signal_sort_key(signal), -index, index, signal)
            if len(heap) < max_findings:
                heapq.heappush(heap, entry)
                continue
            if entry[:4] > heap[0][:4]:
                heapq.heapreplace(heap, entry)
        ranked = sorted(heap, reverse=True)
        return [signal for *_keys, signal in ranked]
...
def signal_sort_key(signal: dict[str, Any]) -> tuple[int, float, int]:
    return (
        SEVERITY_RANK.get(signal["severity"], 0),
        float(signal["score"]),
        int(signal["signal_count"]),
    )
