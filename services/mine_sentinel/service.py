...
MCLOGS_ID_RE = re.compile(r"^[A-Za-z0-9]{3,64}$")
MAX_EXTERNAL_LOG_BYTES = 5 * 1024 * 1024
...
class MineSentinelService:
    ...
    async def report_console_log(
        ...
        result = await self.report_console_log_result(
    ...
    async def report_console_log_result(
        ...
        raw_log = str(source or "").strip()
        if not raw_log:
            return MineSentinelRenderedReport("请提供 mclo.gs 链接或 Minecraft 控制台日志文本")

        raw_url = mclogs_raw_url(raw_log)
        if raw_url:
            try:
                raw_log = await self.io_runner(fetch_mclogs_raw_log, raw_url)
            except Exception as exc:
                self.last_error = f"读取 mclo.gs 日志失败: {exc}"
                return MineSentinelRenderedReport(self.last_error)

        report_server_id = server_id or "minecraft"
        records = parse_minecraft_console_log(
            raw_log,
            server_id=report_server_id,
            server_name=server_name or report_server_id,
        )
        if not records:
            return MineSentinelRenderedReport(
                "日志中没有识别到可总结的 Minecraft 控制台事件；"
                "请确认内容包含类似 [20:52:03] [Server thread/WARN]: ... 的行。"
            )

        window = console_log_window_minutes(records)
        unique_players = len({record.identity for record in records if record.identity})
        window_data = RecentObservationWindow(
            records=records,
...
def mclogs_raw_url(source: str) -> str | None:
    """Return the safe mclo.gs raw URL for a supported log link."""

    tokens = str(source or "").strip().split(maxsplit=1)
    ...
    log_id = ""
    if host == "mclo.gs" and path_parts:
        log_id = path_parts[0]
    elif host == "api.mclo.gs" and len(path_parts) >= 3:
        if path_parts[0] == "1" and path_parts[1] == "raw":
            log_id = path_parts[2]
    if not MCLOGS_ID_RE.fullmatch(log_id):
        return None
    return f"https://api.mclo.gs/1/raw/{log_id}"


def fetch_mclogs_raw_log(raw_url: str) -> str:
    """Fetch a bounded raw mclo.gs log using only the approved API host."""

    parsed = urlparse(raw_url)
    if parsed.scheme != "https" or parsed.netloc.lower() != "api.mclo.gs":
        raise ValueError("仅支持 api.mclo.gs 的 HTTPS raw 日志")
    request = urllib.request.Request(
        raw_url,
        headers={"User-Agent": "MineSentinel/1.0"},
    )
    with urllib.request.urlopen(request, timeout=12) as response:
        content_length = response.headers.get("Content-Length")
        if content_length and int(content_length) > MAX_EXTERNAL_LOG_BYTES:
            raise ValueError("日志超过 5MB 上限，请改为粘贴关键片段")
        data = response.read(MAX_EXTERNAL_LOG_BYTES + 1)
    if len(data) > MAX_EXTERNAL_LOG_BYTES:
        raise ValueError("日志超过 5MB 上限，请改为粘贴关键片段")
