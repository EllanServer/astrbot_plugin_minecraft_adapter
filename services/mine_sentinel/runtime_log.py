...
try:
    ...
    try:
        from mine_sentinel_rs import runtime_log_hints_batch as _rs_runtime_log_hints_batch
    except ImportError:  # pragma: no cover - older native wheel compatibility
        _rs_runtime_log_hints_batch = None
    try:
        from mine_sentinel_rs import runtime_log_time_parts_batch as _rs_runtime_log_time_parts_batch
    except ImportError:  # pragma: no cover - older native wheel compatibility
...
_TIMESTAMP_PARSE_BATCH_SIZE = 1024
...
class MineSentinelRuntimeLogTailer:
    ...
    async def _emit_lines(
        ...
        hints_batch = _runtime_log_hints_batch(lines, self.config.max_line_length)
        timestamps = _parse_log_timestamps_batch(
            lines,
            base_date,
            state.last_timestamp_ms,
        )
        for line, timestamp_ms, hints in zip(lines, timestamps, hints_batch, strict=True):
...
def _backfill_candidates(
    ...
    if source.log_file and not source.logs_dir and not source.root:
...
def _iter_timestamped_lines(
    path: Path,
    max_line_length: int,
    initial_last_ts: int = 0,
    batch_size: int = _TIMESTAMP_PARSE_BATCH_SIZE,
):
    ...
    def flush():
        ...
        if not batch:
            return []
        timestamps = _parse_log_timestamps_batch(batch, base_date, last_ts)
        rows = list(zip(batch, timestamps, strict=True))
...
_OPS_HINT_NEGATIVE_MARKERS: dict[str, tuple[str, ...]] = {
    "database_timeout": (
        "added connection",
        "connection added",
        "hikaripool - starting",
...
def _detect_ops_hint(content: str, level: str) -> dict[str, Any] | None:
    text = str(content or "").lower()
    issue_level = str(level or "").upper() in {"WARN", "WARNING", "ERROR", "FATAL", "SEVERE"}
    if not issue_level and not any(marker in text for marker in _OPS_ISSUE_MARKERS):
        return None
    for code, severity, markers in _OPS_HINT_RULES:
        negative_markers = _OPS_HINT_NEGATIVE_MARKERS.get(code) or ()
        if negative_markers and any(marker in text for marker in negative_markers):
            continue
        hits = [marker for marker in markers if marker in text][:6]
        if hits:
            return {
                "opsHintCode": code,
...
def _python_runtime_log_hints(line: str, max_line_length: int) -> dict[str, Any]:
    ...
    ops_hint = _detect_ops_hint(content, str(hints.get("level") or ""))
    if ops_hint is not None:
        hints.update(ops_hint)
    return hints
...
def _runtime_log_hints_batch(lines: list[str], max_line_length: int) -> list[dict[str, Any]]:
    if not lines:
        return []
    if _HAS_RUST_RUNTIME_HINTS and _rs_runtime_log_hints_batch is not None:
        try:
            hints = _rs_runtime_log_hints_batch(lines, max_line_length)
            if isinstance(hints, list) and len(hints) == len(lines):
                return [
                    item if isinstance(item, dict) else _python_runtime_log_hints(line, max_line_length)
...
def _python_log_time_parts(line: str) -> tuple[str | None, str | None, str | None]:
    ...
    if match:
        return None, match.group("time"), match.group("ms")
    return None, None, None


def _runtime_log_time_parts_batch(
    lines: list[str],
) -> list[tuple[str | None, str | None, str | None]]:
    if not lines:
        return []
    if _HAS_RUST_RUNTIME_HINTS and _rs_runtime_log_time_parts_batch is not None:
        try:
            parts = _rs_runtime_log_time_parts_batch(lines)
...
def _coerce_time_parts(value: Any) -> tuple[str | None, str | None, str | None]:
    if isinstance(value, dict):
        date_text = value.get("date")
        time_text = value.get("time")
        ms_text = value.get("ms")
        return (
            str(date_text) if date_text else None,
            str(time_text) if time_text else None,
            str(ms_text) if ms_text else None,
        )
    if isinstance(value, (list, tuple)):
        parts = list(value[:3])
        while len(parts) < 3:
            parts.append(None)
        return tuple(str(item) if item else None for item in parts)  # type: ignore[return-value]
    return None, None, None


def _parse_log_timestamps_batch(
    lines: list[str],
    base_date: date | None,
    fallback_ms: int = 0,
    ...
    if not lines:
        return []
    current_now_ms = int(time.time() * 1000) if now_ms is None else int(now_ms)
    last_ts = fallback_ms
    timestamps: list[int] = []
    for parts in _runtime_log_time_parts_batch(lines):
        timestamp = _timestamp_from_parts(parts, base_date, last_ts, current_now_ms)
...
def _timestamp_from_parts(
    parts: tuple[str | None, str | None, str | None],
    base_date: date | None,
    fallback_ms: int,
    now_ms: int,
) -> int:
    date_text, time_text, ms_text = parts
    if date_text and time_text:
        return _datetime_to_ms(date_text, time_text, ms_text, fallback_now_ms=now_ms)
    if time_text and base_date:
        timestamp = _date_time_to_ms(base_date, time_text, ms_text, fallback_now_ms=now_ms)
...
def _build_observation(
    ...
    hints = hints if isinstance(hints, dict) else _runtime_log_hints(line, max_line_length)
    ...
    llm_clean_text = str(hints.get("llmCleanText") or content)
    llm_clean_hash = str(hints.get("llmCleanHash") or _clean_text_hash(llm_clean_text))
    llm_quality_score = _as_millis(hints.get("llmQualityScore"))
    quality_flags = [
        str(flag)
        for flag in (hints.get("qualityFlags") or [])
        if str(flag)
    ][:12]
    ...
    ops_hint_markers = [
        str(marker)
        for marker in (hints.get("opsHintMarkers") or [])
        if str(marker)
    ][:8]
    context = {
        "source": "astrbot_runtime_log",
        ...
        "otel": _otel_fields(
            ...
            attributes={
                ...
                "llm.redaction_count": redaction_count,
                "llm.quality_flags": quality_flags,
    ...
    if ops_hint_code:
        context["opsHintCode"] = ops_hint_code
        context["opsHintSeverity"] = ops_hint_severity
        context["opsHintMarkers"] = ops_hint_markers
        context["otel"]["attributes"]["ops.hint_code"] = ops_hint_code
        context["otel"]["attributes"]["ops.hint_severity"] = ops_hint_severity
        context["otel"]["attributes"]["ops.hint_markers"] = ops_hint_markers
    if parsed.params:
...
def _datetime_to_ms(
    ...
    time_text: str,
    ms_text: str | None,
    fallback_now_ms: int | None = None,
) -> int:
    try:
        base = datetime.strptime(f"{date_text} {time_text}", "%Y-%m-%d %H:%M:%S")
    except ValueError:
        return int(time.time() * 1000) if fallback_now_ms is None else fallback_now_ms
    millis = _parse_ms(ms_text)
    return int(base.timestamp() * 1000) + millis


def _date_time_to_ms(
    base_date: date,
    time_text: str,
    ms_text: str | None,
    fallback_now_ms: int | None = None,
...
def _clean_for_llm(line: str) -> tuple[str, int, list[str]]:
    """Return prompt-safe text and lightweight data-quality signals.

    This mirrors a small LLM-training data cleaning pass: remove transport
    escapes/control bytes, redact direct contact/network identifiers, collapse
    whitespace, and mark low-signal lines. The original evidence stays in
    ...
    replacements = [
        ...
        (_UUID_RE, "<uuid>", "redacted_uuid"),
        (_IPV4_RE, "<ip>", "redacted_ip"),
    ...
    if not collapsed:
        flags.append("empty")
    if _has_long_repeated_run(collapsed, 12):
        flags.append("low_signal_repetition")
    if _is_symbol_heavy(collapsed):
        flags.append("low_signal_symbols")
    return collapsed, redaction_count, flags


def _clean_text_hash(text: str) -> str:
    return hashlib.sha1(str(text or "").encode("utf-8")).hexdigest()[:24]


def _llm_quality_score(redaction_count: int, flags: list[str]) -> int:
    score = 100 - min(max(0, redaction_count), 8) * 3
    for flag in flags:
        if flag == "empty":
            score -= 80
            ...
            score -= 2
        elif str(flag).startswith("redacted_"):
            continue
        else:
            score -= 4
...
def _has_long_repeated_run(value: str, threshold: int) -> bool:
    previous = ""
    run = 0
    for ch in value:
        if ch == previous:
