...
### 消息转发配置
  ...
  - **目标会话** 消息转发和消息监听的目标会话，使用 AstrBot 会话 UMO 区分，优先通过 `/sid` 获取，例如 `napcat:GroupMessage:123456789`。也支持 `group:QQ群号`、`qq:QQ号` 简写，运行时会解析到当前 AstrBot 平台 ID。**这个配置标志着 MC 服务器和群聊的互相绑定关系**，群聊中获取 MC 服务器信息和执行指令等，需要服务器和会话绑定
...
### MineSentinel 观察配置
  ...
  - `mine_sentinel.report.delivery_targets` 可独立指定 AstrBot 发送目标，优先使用 `/sid` 输出的完整 UMO（如 `napcat:GroupMessage:123456789`），也支持简写 `group:QQ群号`、`qq:QQ号`、`private:QQ号`；旧 `aiocqhttp:...` UMO 会在运行时解析到当前可用平台 ID，纯数字默认按 QQ 群处理
...
- 目标 QQ 群已通过 `/sid` 拿到 AstrBot UMO，例如 `napcat:GroupMessage:123456789`；也可以在 `target_sessions` 或 `delivery_targets` 中使用简写 `group:123456789`。
...
3. AstrBot 根目录。
4. 要接收 AI 总结的 QQ 群或 UMO。优先提供 `/sid` 输出，例如 napcat:GroupMessage:123456789；也可以提供 group:123456789 或 qq:123456789 简写。
...
8. 配置 AstrBot 插件：
   ...
   - 把 QQ 群写入 message.target_sessions，必要时写入 mine_sentinel.report.delivery_targets；优先写 `/sid` 输出的 UMO，简写 group:/qq: 会自动解析到当前 AstrBot 平台。
