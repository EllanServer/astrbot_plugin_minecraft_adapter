...
### 消息转发配置
  - `message.forward_chat_to_astrbot` 是开关 1：控制游戏内聊天是否实时转发到 QQ/外部会话，默认关闭，不影响 MineSentinel AI 审计采集。
  - 聊天消息格式为，消息发送到其他平台时（如QQ），使用的格式，支持占位符`{player}`和`{message}`，代表发送者游戏ID和消息内容
  - **目标会话** 消息转发和消息监听的目标会话，使用 AstrBot 会话 UMO 区分，优先通过 `/sid` 获取，例如 `napcat:GroupMessage:123456789`、`qq_official:GroupMessage:群 openid` 或你在 AstrBot 平台配置里的 `id:GroupMessage:...`。也支持 `group:QQ群号/openid`、`qq:QQ号/openid` 简写，运行时会解析到当前 AstrBot 平台 ID，兼容 OneBot/NapCat 和官方 QQ Bot。**这个配置标志着 MC 服务器和群聊的互相绑定关系**，群聊中获取 MC 服务器信息和执行指令等，需要服务器和会话绑定
...
### MineSentinel 观察配置
  ...
  - `mine_sentinel.chat.collect_for_ai_audit` 是开关 2：控制是否把游戏内聊天写入 MineSentinel JSONL 并用于 AI 总结/告警；关闭后仍可保留实时转发。
  ...
  - `mine_sentinel.report.delivery_targets` 可独立指定 AstrBot 发送目标，优先使用 `/sid` 输出的完整 UMO（如 `napcat:GroupMessage:123456789`、`qq_official:GroupMessage:群 openid`），也支持简写 `group:QQ群号/openid`、`qq:QQ号/openid`、`private:QQ号/openid`；旧 `aiocqhttp:...` 或 `qq_official:...` 平台名 UMO 会在运行时解析到当前可用平台 ID，纯数字默认按 QQ 群处理
...
- MC 端 Java 插件已开启 `mine_sentinel.enabled`、`observation.enabled`、`chat.collect_for_ai_audit` 和 `metrics.enabled`。
- 目标 QQ 群已通过 `/sid` 拿到 AstrBot UMO，例如 `napcat:GroupMessage:123456789` 或 `qq_official:GroupMessage:群 openid`；也可以在 `target_sessions` 或 `delivery_targets` 中使用简写 `group:123456789`。
...
- 如果报告为空，确认 Java 端 `mine_sentinel.chat.collect_for_ai_audit` 和 `metrics.enabled` 已开启，并且 AstrBot 侧 `mine_sentinel.chat.collect_for_ai_audit` 没有关闭。
...
7. 配置 Java 插件：
   ...
   - `messageForward.enabled` 控制游戏内聊天实时转发到 AstrBot/QQ，默认保持 false；`mine_sentinel.chat.collect_for_ai_audit` 控制是否只收集聊天用于 AI 审计，默认保持 true，两个开关可以分别开启或关闭。
   - 确认 mine_sentinel.enabled、observation.enabled、chat.collect_for_ai_audit、metrics.enabled 为 true。
8. 配置 AstrBot 插件：
   ...
   - `message.forward_chat_to_astrbot` 控制游戏内聊天实时转发到目标会话，默认保持 false；`mine_sentinel.chat.collect_for_ai_audit` 控制聊天 observation 是否进入 AI 审计落盘，默认保持 true。
   - 开启 mine_sentinel.enabled、mine_sentinel.chat.collect_for_ai_audit、storage.enabled、report.enabled、send_as_image、send_full_log_file。
