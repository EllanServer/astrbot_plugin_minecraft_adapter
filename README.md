...
10. 开启 runtime_log、backfill_on_start、loop_filter_enabled、storage、report、send_as_image、send_full_log_file，并保留五段式报告输出。
...
16. 确认报告包含 overall、incidents、community、player_problems、risk_actions 五段，图片报告和 JSONL/JSONL.GZ 附件能发送到目标会话。
...
12. 执行 /ms report now <服务器ID> 30m 和 /ms report now 8h，验证图片报告、文本兜底和完整窗口附件。
13. 检查报告包含 overall、incidents、community、player_problems、risk_actions 五段；正常启动/关闭、Hikari/连接池生命周期、代理握手和 Unknown or incomplete command 不应被误报为管理事件。
