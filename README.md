...
> [!important]
> 本仓库由 EllanServer 独立维护，面向本服适配、MineSentinel 图片巡检报告和 QQ 目标发送等场景持续演进。
...
- `mc report log [服务器ID] <mclo.gs链接|控制台日志文本>` - 分析外部 Minecraft 控制台日志，例如 `mc report log spawn https://mclo.gs/pbfhCaI`
...
### MineSentinel 观察配置
  ...
  - `mc report log` 可直接分析 `mclo.gs` / `api.mclo.gs` 链接或粘贴的 Paper/Leaf 控制台日志，输出同样的五段式 MineSentinel 报告；外部 URL 仅允许安全转换到 `https://api.mclo.gs/1/raw/<id>`，单次抓取上限为 5MB
