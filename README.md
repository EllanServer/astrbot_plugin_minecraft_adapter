...
- Rust 加速 wheel：GitHub Actions -> "Build Rust wheels" 工作流，下载最近一次成功 run 的对应平台 Artifacts。
  直接链接：https://github.com/EllanServer/astrbot_plugin_minesentinel/actions/workflows/rust-wheels.yml
...
```text
你是 Minecraft + AstrBot 升级助手。请把当前 MineSentinel（内部插件 ID：astrbot_plugin_minecraft_adapter）升级为最新版，不要跳过备份、迁移检查和回归验证。
...
2. 切到 main 分支并拉取 https://github.com/EllanServer/astrbot_plugin_minesentinel.git 最新代码；目录名继续保持 astrbot_plugin_minecraft_adapter。
