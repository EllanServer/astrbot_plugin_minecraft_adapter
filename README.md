...
MineSentinel 是一个面向 AstrBot 的 Minecraft 运行日志监控与事件管理报告插件。它只读读取服务器 `latest.log`、历史 `.log` 和 `.log.gz`，通过规则归因、异常检测、训练数据式清洗和 AstrBot 模型能力生成可直接用于值班处置、周期巡检和事件复盘的行动优先报告。

当前项目已经从旧的 Minecraft Adapter 转为纯监控报告 AI：不再包含 Java 端插件、WebSocket、聊天桥、远程命令、玩家绑定或跨端控制链路。GitHub 仓库已使用 MineSentinel 品牌名；内部插件 ID 和安装目录仍保留 `astrbot_plugin_minecraft_adapter`，用于兼容 AstrBot 现有安装与历史数据迁移。
...
- 先做确定性分类和事故聚合，再由 AI 针对重点 issue 复核命中原文前后各 40 条连续日志，生成“初步判断”和结构化处置方案；每个 AI 步骤必须包含检查动作、通过标准和失败后的下一步，末项必须完成恢复验证。证据不足时 AI 可受限扩展上下文，并可按配置调用 AstrBot 网页搜索核对官方资料，但不能修改严重度、计数、玩家、位置、证据、分类或 issue 集合；AI 方案格式不完整、越权或缺少验证时自动回退到确定性规则方案。
- 默认输出“1 页管理摘要 + N 页事件处置单 + 完整证据附件”：摘要直接指出首要事件的准确时间、服务器/后端/世界位置、人物和插件；处置单强制展示日志文件，并把方案拆成检查动作、通过标准和未通过时的升级路径。图片失败时自动回退到同结构文本。
...
raw log line
  ...
  -> incident management    当前状态、处理时限、行动项、验证标准、证据来源
  -> delivery/export        1 页摘要 + N 页处置单、文本兜底、JSONL 附件
```

## 事件管理报告 v3

默认 `report.layout=incident_management`，一份报告由三层组成：

- **管理摘要**：状态、窗口、服务器范围、新发/持续/已恢复/待复核计数，以及按“马上处理、今天处理、留意观察”排序的行动队列；不使用需要额外解释的技术分级代码。
- **事件处置单**：每个活动事件单独一页，固定展示“时间、地点、人物、插件/组件、日志文件”五类事实；随后给出证据定位、依赖检查、具体修复、恢复验证，以及每步的通过标准和失败升级路径。
- **证据附件**：完整窗口 `JSONL` 或 `JSONL.GZ`，正文只展示关键证据；有界采样不会截断落盘附件。

![MineSentinel 事件管理摘要](https://raw.githubusercontent.com/EllanServer/astrbot_plugin_minesentinel/main/docs/report-preview/minesentinel-incident-management-v3-page-1.png)

结构化字段使用 `incident_management.schema_version=3`。周期巡检会把活动事件摘要持久化到数据目录，用相同指纹和连续时间判断 `new`、`ongoing`、`recovered`；状态文件不保存原始聊天或日志正文。即时告警使用相同的行动、负责人和完成标准语言；`/ms report postmortem` 则按复盘标题生成同一套证据化处置页。恢复状态只在事件首次退出活动集的下一个周期出现，之后不再重复占用摘要。

技术事件按故障域拆分：插件配置、数据库、经济资产、网络代理、性能、传送位置、世界区块和认证边界不会仅因发生在同一服务器、相邻几分钟就被压成一个“大事件”。插件名优先从结构化上下文提取，并回退识别日志前缀、调度线程、连接池名和 `plugins/<插件>/...` 路径；无法识别时明确写“未从当前证据识别到具体插件/组件”，不会编造名称。

旧版 `overall`、`incidents`、`community`、`player_problems`、`risk_actions` 五个 section 仍保留在结构化结果的 `legacy_sections` 中，供旧前端和自动化兼容；设置 `report.layout=legacy` 可恢复旧长图，但它不再是默认阅读顺序。
...
统计异常只表示模板出现频率偏离 EWMA/分位数基线，最多把有真实故障语义的事件提升到 `high`；`critical` 必须来自崩溃、OOM、watchdog、服务停止等确定性语义或结构化紧急分类。健康 `INFO` 即使获得高 anomaly score 也不会制造事件或虚增证据数。Vulcan 海量告警按玩家和检查类型聚合到社区观察，不会被误写成服务器崩溃事故或回滚建议。普通“购买飞行/免费飞行”不会仅因出现“飞行”二字变成外挂举报，必须同时存在举报、怀疑、警告、停止、处罚或管理处理语义。
...
正式环境推荐从 [Releases](https://github.com/EllanServer/astrbot_plugin_minesentinel/releases/latest) 安装。下载 `astrbot_plugin_minecraft_adapter.zip`，解压到 AstrBot 插件目录下的 `astrbot_plugin_minecraft_adapter` 目录，再使用 AstrBot 实际运行的 Python 安装依赖：

```bash
pip install -r astrbot_plugin_minecraft_adapter/requirements.txt
```

固定直链：`https://github.com/EllanServer/astrbot_plugin_minesentinel/releases/latest/download/astrbot_plugin_minecraft_adapter.zip`

需要跟踪 `main` 分支时，可使用源码安装：

```bash
git clone https://github.com/EllanServer/astrbot_plugin_minesentinel.git astrbot_plugin_minecraft_adapter
pip install -r astrbot_plugin_minecraft_adapter/requirements.txt
```
...
```text
你是 Minecraft + AstrBot 部署助手。请从 GitHub 最新正式 Release 部署 MineSentinel 监控管理报告 AI，不要跳过版本确认、校验、备份、配置和验证。

GitHub 仓库：https://github.com/EllanServer/astrbot_plugin_minesentinel
最新正式版 API：https://api.github.com/repos/EllanServer/astrbot_plugin_minesentinel/releases/latest
Release 页面：https://github.com/EllanServer/astrbot_plugin_minesentinel/releases/latest
必须下载的资产：astrbot_plugin_minecraft_adapter.zip、SHA256SUMS.txt
可选资产：与 AstrBot 实际 Python、操作系统和 CPU 架构匹配的 mine_sentinel_rs wheel

发布规则：
1. 只使用非 draft、非 prerelease 的最新 Release；记录 tag、发布时间和资产名称。
2. 不要把 GitHub 自动生成的 Source code zip/tar.gz 当成插件安装包，也不要默认使用 main 分支或 Actions 临时 Artifacts。
3. 插件安装目录必须保持为 astrbot_plugin_minecraft_adapter；Release ZIP 内是插件根文件，需要解压到该目录中。
4. 不要在目标机器本地编译 Rust；没有完全匹配的 wheel 时保持纯 Python 降级运行。
...
2. 调用最新正式版 API，确认 HTTP 成功、draft=false、prerelease=false，并读取 tag_name 和 assets；找不到必需资产时停止，不要猜测下载地址。
3. 下载插件 ZIP 和 SHA256SUMS.txt 到临时目录，按清单校验 SHA-256；校验失败立即停止并删除下载文件。
4. 安装前把旧插件目录、配置文件和 plugin_data/mine_sentinel 备份到带时间戳的 backup 目录；如果没有旧数据也要说明。
5. 把 ZIP 解压到临时 staging 目录，确认存在 metadata.yaml、main.py、requirements.txt、_conf_schema.json、handlers 和 services，并确认 metadata.yaml 的版本与 Release tag 一致。
6. 停止 AstrBot 后再把 staging 内容安装到 astrbot_plugin_minecraft_adapter；不要覆盖或删除 plugin_data/mine_sentinel 和 AstrBot 配置。
7. 使用 AstrBot 实际运行的 Python 执行 pip install -r requirements.txt。
8. 可选启用 Rust 加速：从同一个 Release 选择完全匹配的 wheel 并执行 pip install --force-reinstall --no-deps <wheel>.whl；安装后 import mine_sentinel_rs 验证，失败则卸载不兼容 wheel 并记录为纯 Python 降级。
9. 在 mine_sentinel.runtime_log.sources 写入服务器 root、logs_dir 或 log_file；Velocity 群组服要把 Velocity 和所有后端服分别写成 source。
10. 开启 runtime_log、backfill_on_start、loop_filter_enabled、storage、report、send_as_image、send_full_log_file；设置 report.layout=incident_management、report.lifecycle_enabled=true，并保持完整证据附件开启。
11. 自动检测 AstrBot 已配置 bot 平台；读取 <AstrBot 根目录>/data/cmd_config.json 时用 utf-8-sig，识别 enable=true 的 aiocqhttp / qq_official / qq_official_webhook。若用户提供的是 group: 或 qq: 简写，优先解析到可用 QQ 平台；若平台不唯一，列出候选让我选择。
12. 全局投递目标写入 mine_sentinel.report.delivery_targets；单服单独投递写入 source.delivery_targets 或 source.target_sessions。send_to_target_sessions 默认保持 true。
13. 按日志量选择性能档位：
...
14. 重启 AstrBot 后执行 /ms monitor status，确认日志源数量、轮询状态、observation/export 目录和 io_workers 生效。
15. 触发或等待一条 Minecraft 日志后执行 /ms report now <服务器ID> 30m；再执行 /ms report now 8h 验证全局报告。
16. 确认首图为管理摘要，行动队列位于事件索引之前；每个活动事件有独立处置单，明确列出时间、地点、人物、插件/组件和日志文件，并至少提供“检查动作、通过标准、未通过怎么办”；JSONL/JSONL.GZ 证据附件能发送到目标会话。
17. 最后汇总 Release tag、ZIP 校验结果、安装文件、备份位置、日志源 server_id、性能档位、投递目标、Rust 加速状态、验证结果和需要我手动确认的事项。
```

### 已安装升级

```text
你是 Minecraft + AstrBot 升级助手。请把当前 MineSentinel（内部插件 ID：astrbot_plugin_minecraft_adapter）升级到 GitHub 最新正式 Release，不要跳过版本比较、校验、备份、回滚准备、迁移检查和回归验证。
...
升级规则：
1. 只使用非 draft、非 prerelease 的最新 Release，不要默认拉取 main，也不要使用 Source code zip/tar.gz 替代插件安装包。
2. 读取当前 metadata.yaml version，与最新 tag 按语义版本比较；当前版本相同或更高时停止替换并报告结果。
3. 插件目录名继续保持 astrbot_plugin_minecraft_adapter；不要在目标机器本地编译 Rust，缺少匹配 wheel 时允许纯 Python 降级。

升级前先记录并备份：
1. 当前插件目录、metadata.yaml 版本；如果是 git 安装，再记录分支、HEAD、未提交改动和远端地址。
...
执行要求：
1. 调用最新正式版 API 并比较版本；确认需要升级后，下载插件 ZIP 和 SHA256SUMS.txt 到临时目录并完成 SHA-256 校验。
2. 如果旧插件目录有未提交改动，先生成 diff 备份；同时制作可直接恢复的完整插件目录备份，不要直接丢弃本地修改。
3. 把 ZIP 解压到独立 staging 目录，验证必需文件和 metadata.yaml 版本与 Release tag 一致；不要直接覆盖正在运行的插件目录。
4. 停止 AstrBot，保留 AstrBot 配置和 plugin_data/mine_sentinel，用 staging 目录替换插件代码；不要把旧目录中已经删除的源码文件继续混入新版本。
5. 用 AstrBot 实际运行的 Python 执行 pip install -r requirements.txt。
6. 可选升级 Rust 加速 wheel：只安装同一个 Release 中完全匹配环境的 wheel，使用 import mine_sentinel_rs 验证；失败时回退纯 Python，不要阻塞启动。
7. 保留 mine_sentinel 下的日志源、投递目标、报告周期和导出配置；移除旧 Minecraft Adapter 的 Java 端插件、WebSocket、聊天桥、远程命令、玩家绑定等废弃配置。
8. 检查旧路径 plugin_data/astrbot_plugin_minecraft_adapter/mine_sentinel 是否已自动迁移到 plugin_data/mine_sentinel；迁移前后文件数量和最新 observation/export 文件要对得上。
9. 如果发现旧 .idx 偏移索引来自早期版本，优先备份后删除让新版重建；不想删除时把 mine_sentinel.storage.trust_legacy_index 设为 false 进入保守扫描模式。
10. 按日志量重新选择性能档位：
...
11. 重启 AstrBot 后执行 /ms monitor status，确认日志源、轮询、backlog、异常检测、报告投递目标都正常。
12. 执行 /ms report now <服务器ID> 30m、/ms report now 8h 和 /ms report postmortem <服务器ID> 24h，验证管理摘要、事件处置单、文本兜底和完整窗口附件。
13. 检查 report.layout=incident_management；确认行动队列先于事件索引，处置单带事件 ID、生命周期、准确时间、服务器/后端/世界位置、人物、插件/组件、日志文件、检查动作、通过标准和失败升级路径；正常启动/关闭、Hikari/连接池生命周期、代理握手和 Unknown or incomplete command 不应被误报为管理事件。旧五段 section 只作为兼容数据保留。
14. 任一启动或核心验证失败时，停止 AstrBot，恢复完整插件目录和配置备份，再启动旧版本并记录失败日志。
15. 最后汇总旧版本、新 Release tag、ZIP 校验结果、备份位置、迁移与回滚状态、性能档位、Rust 加速状态、验证结果和仍需人工确认的风险。
```
...
- `/ms report postmortem [服务器ID] [24h]`：生成事件复盘；不传窗口时默认至少回看 24 小时。
...
这些测试会确认显式 `log_file` 只读取指定文件，pbfhCaI 样本能识别 `bug`、`plugin`、`network`、`economy`，v3 事件状态可从新发演进到持续和已恢复，旧五段 section id 保持兼容，并过滤 Hikari/AstrbotAdapter/CMI 生命周期噪声；v54kwmi 样本用于验证 10k 行窗口、4202 条 Vulcan 在在线 loop filter 前后计数一致、连接池 WARN 按时间切分、玩家反馈语义分流和统计异常严重度上限。
